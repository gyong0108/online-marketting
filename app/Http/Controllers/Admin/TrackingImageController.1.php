...
class TrackingImageController extends Controller
{
    ...
    public function track($img, Request $request)
    {
        ...
        // $page = Page::where('hash', $request->input('key'))->first();
        $campaign = Campaign::where('id', $request->input('key'))->first();
        //right?i hope
        if($campaign==null) {
        ...
        $storagePath = public_path('FSOM_Logo_h60px_invert.png'); //This we need in public
