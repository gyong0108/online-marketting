    ...
    public function testCreateUser()
    {
        ...
        $this->browse(function (Browser $browser) use ($admin, $user, $relations) {
            $browser->loginAs($admin)
                ...
                ->check("approved")
                ->check("premium")
                ->type("stripe_customer_id", $user->stripe_customer_id)
                ->press('Save')
                ...
                ->assertChecked("premium")
                ->assertSeeIn("tr:last-child td[field-key='stripe_customer_id']", $user->stripe_customer_id)
                ->logout();
        });
    }
    ...
    public function testEditUser()
    {
        ...
        $this->browse(function (Browser $browser) use ($admin, $user, $user2, $relations) {
            $browser->loginAs($admin)
                ...
                ->check("approved")
                ->check("premium")
                ->type("stripe_customer_id", $user2->stripe_customer_id)
                ->press('Update')
                ->assertRouteIs('admin.users.index')
                ...
                ->assertChecked("approved")
                ->assertChecked("premium")
                ->assertSeeIn("tr:last-child td[field-key='stripe_customer_id']", $user2->stripe_customer_id)
                ->logout();
        });
    }
    ...
    public function testShowUser()
    {
        ...
        $this->browse(function (Browser $browser) use ($admin, $user, $relations) {
            $browser->loginAs($admin)
                ...
                ->assertNotChecked("premium")
                ->assertSeeIn("td[field-key='stripe_customer_id']", $user->stripe_customer_id)
                ->logout();
        });
    }
