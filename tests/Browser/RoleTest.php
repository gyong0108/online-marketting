    ...
    public function testCreateRole()
    {
        ...
        $this->browse(function (Browser $browser) use ($admin, $role, $relations) {
            $browser->loginAs($admin)
                ...
                ->type("price", $role->price)
                ->type("stripe_plan_id", $role->stripe_plan_id)
                ->press('Save')
                ...
                ->assertSeeIn("tr:last-child td[field-key='price']", $role->price)
                ->assertSeeIn("tr:last-child td[field-key='stripe_plan_id']", $role->stripe_plan_id)
                ->logout();
        });
    }
    ...
    public function testEditRole()
    {
        ...
        $this->browse(function (Browser $browser) use ($admin, $role, $role2, $relations) {
            $browser->loginAs($admin)
                ...
                ->select('select[name="permission[]"]', $relations[0]->id)
                ->select('select[name="permission[]"]', $relations[1]->id)
                ->type("price", $role2->price)
                ->type("stripe_plan_id", $role2->stripe_plan_id)
                ->press('Update')
                ->assertRouteIs('admin.roles.index')
                ->assertSeeIn("tr:last-child td[field-key='title']", $role2->title)
                ->assertSeeIn("tr:last-child td[field-key='permission'] span:first-child", $relations[0]->title)
                ->assertSeeIn("tr:last-child td[field-key='permission'] span:last-child", $relations[1]->title)
                ->assertSeeIn("tr:last-child td[field-key='price']", $role2->price)
                ->assertSeeIn("tr:last-child td[field-key='stripe_plan_id']", $role2->stripe_plan_id)
                ->logout();
        });
    }
    ...
    public function testShowRole()
    {
        ...
        $this->browse(function (Browser $browser) use ($admin, $role, $relations) {
            $browser->loginAs($admin)
                ...
                ->assertSeeIn("tr:last-child td[field-key='permission'] span:first-child", $relations[0]->title)
                ->assertSeeIn("tr:last-child td[field-key='permission'] span:last-child", $relations[1]->title)
                ->assertSeeIn("td[field-key='price']", $role->price)
                ->assertSeeIn("td[field-key='stripe_plan_id']", $role->stripe_plan_id)
                ->logout();
        });
    }
